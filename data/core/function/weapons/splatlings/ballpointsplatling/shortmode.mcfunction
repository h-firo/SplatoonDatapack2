...
summon armor_stand ^ ^ ^2.0 {Tags:["shot$(team)","ink","owner$(num)","general","sn$(shotNum)"],Invisible:true,Silent:true,Small:true,data:{team:$(team),owner:$(num),damage:28,shotNum:$(shotNum),damageAT:0,objDamage:$(obj)},attributes:[{id:"minecraft:gravity",base:1},{id:"minecraft:scale",base:0.1}]}
...
$data merge storage shot_temp: {$(num):{Speed:0.0008}}
...
scoreboard players remove @s ink 4
scoreboard players set @s shotDelay 0
scoreboard players set @s splatlingsShotDelay 1
