$particle dust{color:[$(r),$(g),$(b)],scale:1} ~ ~ ~ 0.1 0.1 0.1 0 1 force
...
$execute if score @s explosionTimer matches ..5 as @e[distance=..1.5,tag=player,tag=!asAttack$(bombNum)] unless entity @s[level=$(owner)] unless score @s team matches $(team) run function core:damages/remove_health {value:49,type:"angleshooter",killer:$(owner)}
...
#マーキング
$execute if score @s explosionTimer matches ..5 as @e[distance=..1.5,tag=player,tag=asAttack$(bombNum)] run tag @s add Marking
$execute if score @s explosionTimer matches ..5 as @e[distance=..1.5,tag=player,tag=asAttack$(bombNum),tag=Marking] run tag @s add Mark$(team)
