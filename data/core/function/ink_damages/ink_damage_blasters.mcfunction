#ブラスター
$function core:damage {distance:15,num:$(shotNum),team:$(team),damage:$(damage),objDamage:$(objDamage),volume:1,objNum:$(objNum),owner:$(owner),type:"main"}
...
$execute if entity @e[tag=attack$(shotNum)] run playsound entity.armor_stand.break master @a ~ ~ ~ 1 1
$execute if entity @e[tag=attack$(shotNum)] run playsound entity.generic.explode master @a ~ ~ ~ 0.4 2
