...
summon armor_stand ^ ^ ^2.0 {Tags:["shot$(team)","ink","owner$(num)","general","sn$(shotNum)"],Invisible:true,Silent:true,Small:true,data:{team:$(team),owner:$(num),damage:28,shotNum:$(shotNum),damageAT:0,objDamage:$(obj)},attributes:[{id:"minecraft:gravity",base:$(Gravity)},{id:"minecraft:scale",base:0.1}]}
...
$data merge storage shot_temp: {$(num):{Speed:0.0017}}
$execute as @n[type=armor_stand,tag=owner$(num),tag=sn$(shotNum)] at @s run function core:weapons/splatlings/shot_as with storage shot_temp: $(num)
playsound block.stone.place master @a ~ ~ ~ 0.8 1.5
scoreboard players set @s delay 9
scoreboard players remove @s ink 6
scoreboard players set @s shotDelay 0
scoreboard players set @s splatlingsShotDelay 2
