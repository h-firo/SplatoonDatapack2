...
summon armor_stand ^ ^ ^2.0 {Tags:["shot$(team)","ink","owner$(num)","blasters","sn$(shotNum)"],Invisible:true,Silent:true,Small:true,data:{team:$(team),owner:$(num),damage:$(Damage),shotNum:$(shotNum),damageAT:$(DamageAT),objDamage:$(obj),radius:$(BlastRadius),blastDamage:$(BlastDamage),duration:$(BlastDuration)},attributes:[{id:"minecraft:gravity",base:0},{id:"minecraft:scale",base:0.1}]}
...
$execute as @n[type=armor_stand,tag=owner$(num),tag=sn$(shotNum)] at @s run function core:weapons/blasters/shot_as with storage shot_temp: $(num)
