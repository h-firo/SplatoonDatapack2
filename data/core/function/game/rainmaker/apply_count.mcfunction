$scoreboard players set team$(team) areaCount 100
$scoreboard players operation team$(team) areaCount -= dist rainmakerGoalPos

#0~100(チェックポイント未到達時はチェックポイントの数)にする
$execute if score team$(team) areaCount matches ..0 run scoreboard players set team$(team) areaCount 0
...
$execute if score team$(team) areaCount matches 100.. run scoreboard players set team$(team) areaCount 99