...
#攻撃判定
$execute as @e[tag=player,distance=..1] unless score @s team matches $(team) run function core:damages/remove_health {value:5,type:"wavebreaker",killer:$(owner)}

#マーキング
#execute as @e[tag=player,distance=..1] if predicate core:is_onground run tag @s add Marking
$execute as @e[tag=player,distance=..1] unless score @s team matches $(team) run tag @s add Marking
